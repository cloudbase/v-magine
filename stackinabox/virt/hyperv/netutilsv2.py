...
class NetworkUtilsV2(netutils.NetworkUtils):
    ...
    def __init__(self):
        super(NetworkUtilsV2, self).__init__()
    ...
    def set_vnic_port_security(self, switch_port_name,
                               ...
                               allow_teaming=False,
                               monitor_mode=0):
        port_alloc = self._get_switch_port_allocation_check(switch_port_name)

        sec_settings_list = port_alloc.associators(
            wmi_result_class=self._PORT_SECURITY_SET_DATA)

        if sec_settings_list:
            sec_settings = sec_settings_list[0]
        else:
            sec_settings = self._get_default_setting_data(
                self._PORT_SECURITY_SET_DATA)

        svc = self._conn.Msvm_VirtualSystemManagementService()[0]

        sec_settings.AllowMacSpoofing = allow_mac_spoofing
        sec_settings.EnableDhcpGuard = enable_dhcp_guard
        sec_settings.EnableRouterGuard = enable_router_guard
        sec_settings.AllowIeeePriorityTag = allow_ieee_priority_tag
        sec_settings.AllowTeaming = allow_teaming
        sec_settings.MonitorMode = monitor_mode

        if sec_settings_list:
            (job_path, out, ret_val) = svc.ModifyFeatureSettings(
                [sec_settings.GetText_(1)])
        else:
            (job_path, out, ret_val) = svc.AddFeatureSettings(
                port_alloc.path_(), [sec_settings.GetText_(1)])
        self._check_job_status(ret_val, job_path)
    ...
    def set_vswitch_port_vlan_id(self, vlan_id, switch_port_name,
        ...
        if trunk_vlan_ids:
            vlan_settings.OperationMode = self._OPERATION_MODE_TRUNK
            vlan_settings.TrunkVlanIdArray = trunk_vlan_ids
            vlan_settings.NativeVlanId = native_vlan_id
        elif vlan_id:
            vlan_settings.AccessVlanId = vlan_id
            vlan_settings.OperationMode = self._OPERATION_MODE_ACCESS

        if trunk_vlan_ids or vlan_id:
...
class NetworkUtilsV2R2(NetworkUtilsV2):
